...
void oled_init(void);     
void oled_fill(uint8 dat);//OLED清屏
...
void oled_p8x16str(uint8 x,uint8 y,const int8 ch[]);//显示字符串
void oled_uint16(uint8 x, uint8 y, uint16 num);//显示数据
void oled_int16(uint8 x, uint8 y, int16 num);
void oled_printf_int32(uint16 x,uint16 y,int32 dat,uint8 num);
void oled_printf_float(uint16 x,uint16 y,double dat,uint8 num,uint8 pointnum);//显示浮点数
